simgraph_graph <- function(result, nsims = 10, dsigma = 50, dims = 250, labls = TRUE, steps = TRUE,
  ...
  result <- result %>% dplyr::slice_head(n = nsims)
  ...
  ggp <- ggplot::ggplot(result[1:nsims,]) + aes(x = f0x, y = f0y, xend = f1ax, yend = f1ay)

  if (histogram == TRUE){

    ggp <- ggplot::ggplot(result[1:nsims,]) + aes(x = kindist)
    if (freqpoly == FALSE){
  ...
  if (pinwheel == TRUE){
    ...
    if (labls != FALSE & lengthlabs != FALSE){
      ggp <- ggp + geom_label_repel(mapping = aes(x = k2x, y = k2y, label = paste(kindist, "m")), colour = "black",
  ...
  if (labls == TRUE & steps == TRUE){
    ...
    if (category %in% c("PO", "FS", "HS", "AV", "GG", "HAV", "GGG", "1C", "1C1", "2C", "GAV")){
      ggp <- ggp + geom_label_repel(mapping = aes(x = f1ax, y = f1ay), alpha = 0.7, colour = "black", label = "1",
                                    size = 2, label.padding = unit(0.1, "lines"), box.padding = unit(0.01, "lines"))}
    if (category %in% c("FS", "HS", "AV", "HAV", "1C", "1C1", "2C", "GAV")){
      ggp <- ggp + geom_label_repel(mapping = aes(x = f1bx, y = f1by), alpha = 0.7, colour = "black", label = "1",
                                    size = 2, label.padding = unit(0.1, "lines"), box.padding = unit(0.01, "lines"))}
    if (category %in% c("AV", "GG", "HAV", "GGG", "1C", "1C1", "2C", "GAV")){
      ggp <- ggp + geom_label_repel(mapping = aes(x = f2ax, y = f2ay), alpha = 0.7, colour = "green4", label = "2",
                                    size = 2, label.padding = unit(0.1, "lines"), box.padding = unit(0.01, "lines"))}
    if (category %in% c("1C", "1C1", "2C")){
      ggp <- ggp + geom_label_repel(mapping = aes(x = f2bx, y = f2by), alpha = 0.7, colour = "green4", label = "2",
                                    size = 2, label.padding = unit(0.1, "lines"), box.padding = unit(0.01, "lines"))}
    if (category %in% c("GGG", "1C1", "2C", "GAV")){
      ggp <- ggp + geom_label_repel(mapping = aes(x = f3ax, y = f3ay), alpha = 0.7, colour = "purple", label = "3",
                                    size = 2, label.padding = unit(0.1, "lines"), box.padding = unit(0.1, "lines"))}
    if (category %in% c("2C")){
      ggp <- ggp + geom_label_repel(mapping = aes(x = f3bx, y = f3by), alpha = 0.7, colour = "purple", label = "3",
                                    size = 2, label.padding = unit(0.1, "lines"), box.padding = unit(0.01, "lines"))}}

  if (lengths == TRUE & lengthlabs == TRUE){
    ggp <- ggp + geom_label_repel(mapping = aes(x = kinmidx, y = kinmidy, label = paste(kindist, "m")), colour = "black",
                                  size = 3, hjust = 0.5, vjust = 0.5, alpha = 1, box.padding = unit(0.01, "lines"),
                                  label.padding = unit(0.1, "line"), fontface = "bold")}
...
simgraph_basic <- function(simdata, nsims = 10, dsigma = 25, dims = 250,
                           labls = F, steps = T, moves = T, shadows = F,
                           show_area = T, category = "2C",
                           lengths = T, lengthlabs = T) {
  return(simgraph(nsims = nsims, dsigma = dsigma, dims = dims, labls = labls, steps = steps, moves = moves, shadows = shadows,
                  show_area = show_area, category = category, lengths = lengths, lengthlabs = lengthlabs))
}

simgraph_centred_graph <- function(nsims = 5, dsigma = 25, dims = 250,
                                   labls = F, steps = T, moves = T, shadows = F,
                                   show_area = T, category = "2C",
...
simgraph_pinwheel <- function(nsims = 200, dsigma = 25, labls = F, category = "2C"){
  return(simgraph(nsims = nsims, dsigma = dsigma, labls = labls, category = category, pinwheel = TRUE))
}

simgraph_scatter <- function(nsims = 500, dsigma = 25, category = "2C"){
  return(simgraph(nsims = nsims, dsigma = dsigma, category = category, scattered = TRUE))
}

simgraph_indv <- function(dsigma = 25, dims = 250, scalefactor = 4, scaled = T,
                          labls = F, steps = T, moves = T, shadows = F,
                          show_area = T, category = "2C",
                          lengths = T, lengthlabs = T) {
  newtitle = paste0(category, " Dispersal: sigma ", dsigma, "m")
  if (scaled == TRUE){
    return(simgraph(nsims = 1, dsigma = dsigma, dims = dims, labls = labls, steps = steps, moves = moves, shadows = shadows,
                    show_area = show_area, category = category, lengths = lengths, lengthlabs = lengthlabs, centred = T) +
...
simgraph_histogram <- function(nsims = 500, dsigma = 25, category = "2C", binwidth = dsigma / 3){
  return(simgraph(nsims = nsims, dsigma = dsigma, category = category, histogram = TRUE, binwidth = binwidth))
}

simgraph_freqpoly <- function(nsims = 5000, dsigma = 25, category = "2C", binwidth = dsigma / 3){
  return(simgraph(nsims = nsims, dsigma = dsigma, category = category, histogram = TRUE, binwidth = binwidth, freqpoly = TRUE))
}
